...
namespace EMart.ViewModel.Registeration
{
    public class RegisterationCrudViewModel
    {
        ...
        public int Admin_Register(RegisterationViewModel verifiedUser)
        {
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("Admin_Registration", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@UserImageUrl", verifiedUser.Admin.UserImageUrl);
                    cmd.Parameters.AddWithValue("@UserPhoneNumber", verifiedUser.Admin.PhoneNumber);
                    cmd.Parameters.AddWithValue("@UserAddress", verifiedUser.Admin.UserAddress);
        ...
        public string Customer_register(RegisterationViewModel verifiedUser)
        {
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("User_Registration", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@UserImageUrl", verifiedUser.Customer.UserImageUrl);
                    cmd.Parameters.AddWithValue("@UserPhoneNumber", verifiedUser.Customer.PhoneNumber);
                    cmd.Parameters.AddWithValue("@UserAddress", verifiedUser.Customer.UserAddress);
        ...
        public User userlogin(User log, int f_key)
        {
            //encrypted password
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                using (SqlCommand cm = new SqlCommand("Login", con))
                {
                    ...
                    if (dr.Read())
                    {
                        ...
                        log.UserAddress = dr["UserAddress"].ToString();
                    }
                    //else
                    //{
                    //}
         ...
         public User EditUser(int id)
         {
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                using (SqlCommand cm = new SqlCommand("sp_User_EditUser", con))
                {
                    ...
                    if (dr.Read())
                    {
                        user.FirstName = dr["UserFirstName"].ToString();
                        user.LastName = dr["UserLastName"].ToString();
                        user.UserAddress = dr["UserAddress"].ToString();
                        user.UserPassword = dr["UserPassword"].ToString();
                        user.ConfirmPassword = dr["UserPassword"].ToString();
                        user.UserId =int.Parse(dr["UserId"].ToString());

                    }

                    //decrypted password
                    string dec = user.UserPassword;
                    ...
                    //{
                    //}
                    user.UserPassword = dec_pass;
                    user.ConfirmPassword = dec_pass;
                }
            }
            return user;             
        }
        ...
        {
            //encrypted password
            string enc = user.UserPassword;
            byte[] encode = new byte[enc.Length];
            encode = Encoding.UTF8.GetBytes(user.UserPassword);
            string enc_password = Convert.ToBase64String(encode);
            string message = "";
            string connection = ConfigurationManager.ConnectionStrings["E_MART"].ConnectionString;
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                {
                    cm.CommandType = System.Data.CommandType.StoredProcedure;
                    cm.Parameters.AddWithValue("@User_FirstName", user.FirstName);
                    cm.Parameters.AddWithValue("@User_LastName", user.LastName);
        ...
        public string ShopkeeperRegister(RegisterationViewModel verifiedUser)
        {
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("ShopManagerRegistration", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@UserImageUrl", verifiedUser.ShopManager.UserImageUrl);
                    cmd.Parameters.AddWithValue("@UserPhoneNumber", verifiedUser.ShopManager.PhoneNumber);
                    cmd.Parameters.AddWithValue("@UserAddress", verifiedUser.ShopManager.UserAddress);
